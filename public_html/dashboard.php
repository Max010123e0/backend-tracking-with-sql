...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    section.card, aside.card { background: #fff; border-radius: 8px;
            box-shadow: 0 1px 3px rgba(0,0,0,.08); padding: 20px; margin-bottom: 16px; }
    section.card h2, aside.card h2 { margin: 0 0 8px; font-size: 1.1rem; color: #111827; }
    section.card p, aside.card p  { margin: 0; color: #6b7280; }
    .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px,1fr)); gap: 16px; }
    section-card { display: block; background: #fff; border-radius: 8px; box-shadow: 0 1px 3px rgba(0,0,0,.08);
...
?>
      <section-card class="<?= ltrim($lockClass) ?>" style="border-left-color:<?= $sec['color'] ?>">
